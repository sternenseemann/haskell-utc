  ...
  = do -- calculate the single digits
       ...
       return $ mconcat
        ...
        , case os of
            ...
            Just 0  -> BS.char7 'Z'
            Just o  -> let oh1 = fromIntegral $ abs (truncate o `quot` 36000          `rem` 10 :: Integer)
                           oh0 = fromIntegral $ abs (truncate o `quot` 3600           `rem` 10 :: Integer)
                           om1 = fromIntegral $ abs (truncate o `rem`  3600 `quot` 10 `rem` 10 :: Integer)
                           om0 = fromIntegral $ abs (truncate o `rem`  3600           `rem` 10 :: Integer)
                       in  mconcat
                             [ if o < 0
                                 then BS.char7 '-'
                                 else BS.char7 '+'
